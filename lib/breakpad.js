...
export async function symbolicateFrame(stream, offsets) {
  const matches = offsets.map(() => null);
  const publicMatches = offsets.map(() => null);
  const lineWatchers = [];
  const files = new Map();
  ...
  return matches.map((match, i) => {
    if (match === null) {
      ...
      if (publicMatch) {
        return {
          name: Buffer.concat(publicMatch.name).toString(),
    ...
    return {
      name: Buffer.concat(name).toString(),
      addr,
      size,
      file: file ? Buffer.concat(file).toString() : null,
      line,
    };
  });
}
